...
class TurboQuantCompressor:
    ...
    def compress_value_batch(self, x: mx.array, rotations: mx.array) -> dict:
        """Batched value compression: x (H, S, D), rotations (H, D, D) -> indices, norms."""
        x_f32 = x.astype(mx.float32)
        x_norm = mx.linalg.norm(x_f32, axis=-1, keepdims=True)
        x_unit = x_f32 / mx.maximum(x_norm, mx.array(1e-8))

        Q_T = mx.swapaxes(rotations, -2, -1)  # (H, D, D)
        x_rot = mx.matmul(x_unit, Q_T)  # (H, S, D)
...
class TurboQuantKVCache:
    ...
    def update_and_fetch(
        ...
        # Batched compression: all heads in a single GPU dispatch
        comp_k = self.compressor.compress_value_batch(k_batch, self.rotation_matrices)
        comp_v = self.compressor.compress_value_batch(v_batch, self.rotation_matrices)

        new_compressed_keys = {
            "indices": comp_k["indices"].astype(mx.uint8),
            "x_norm": comp_k["x_norm"].astype(mx.float16),
        }
        new_compressed_vals = {
            "indices": comp_v["indices"].astype(mx.uint8),
            "x_norm": comp_v["x_norm"].astype(mx.float16),
        }
