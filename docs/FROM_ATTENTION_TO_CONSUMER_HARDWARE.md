...
|--------|----------|-------------|-------|
| FP16 TFLOPS | [PLACEHOLDER] | ~27 | [PLACEHOLDER] |
| FP32 TFLOPS | [PLACEHOLDER] | ~14 | [PLACEHOLDER] |
| Memory bandwidth (GB/s) | [PLACEHOLDER] | 546 | [PLACEHOLDER] |
| MLX dispatch overhead (μs) | [PLACEHOLDER] | — | — |
| Mojo dispatch overhead (μs) | [PLACEHOLDER] | — | — |
...
[PLACEHOLDER: Forward rotation (prefill) and inverse rotation (decode) results — structured vs dense speedup, GB/s, roofline fraction.]

**KV compression.** Codebook-based vector quantisation with 3-bit indices.

[PLACEHOLDER: Compress and decompress GB/s, reconstruction RMSE.]
...
[PLACEHOLDER: Three-variant results for IsoQuant and TurboQuant pipelines. Per-sub-operation cost breakdown table. Framework-fused vs unfused speedup. Hand-fused vs framework-fused speedup (MLX only; Mojo hand-fused is CPU-only reference due to Tier 3 limitations).]

### 10c.6 Precision analysis

Precision is validated via a kernel-chain simulation (MatMul → RoPE → Softmax → MatMul → output projection) comparing MLX float32 against numpy float64 reference. The equivalent perplexity delta is reported as an **informative proxy, not a bound** — the relationship between partial-pipeline and full-pipeline divergence is non-monotonic (residual connections and LayerNorm may dampen or amplify kernel-level errors).

[PLACEHOLDER: Per-kernel RMSE table, perplexity delta (%), 0.5% threshold (Dettmers et al. 2022).]

### 10c.7 Energy efficiency

[PLACEHOLDER: TFLOPS/W comparison per kernel class. Note that sub-second kernel runs have limited power precision — `powermetrics` samples at ~250 ms intervals. Energy charts aggregate to kernel-class level for meaningful averages.]
...
[PLACEHOLDER: ALU utilisation, memory bandwidth utilisation, occupancy (waves/EU), and stall-cycle breakdown for top-3 kernels, both frameworks. See `docs/gpu-profiling-protocol.md` for methodology.]
...
[PLACEHOLDER: Stacked-bar chart showing kernel fractions. If the benchmarked kernels account for less than 50% of total decode time, this is stated prominently — the remaining time is spent in operations outside the benchmark scope (e.g., residual connections, LayerNorm, embedding lookup, sampling).]
