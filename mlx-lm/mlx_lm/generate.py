...
def setup_arg_parser():
    ...
    parser.add_argument(
        "--expert-offload",
        action="store_true",
        help="Nemotron-H only: routed expert weights loaded on demand (LRU).",
    )
    parser.add_argument(
        "--max-resident-experts",
        type=int,
        default=16,
        help="LRU capacity for routed experts when --expert-offload is set.",
    )
    parser.add_argument(
        "--expert-offload-dir",
        type=str,
        default=None,
        help="Directory containing safetensors shards (default: model path).",
    )
    parser.add_argument(
        "--memory-mode",
        type=str,
        default=None,
        help='Optional profile: use "120b-32gb" to enable --expert-offload and turboquant KV.',
    )
    parser.add_argument(
        "--prefill-step-size",
        type=int,
        default=2048,
        help="Step size for processing the prompt during prefill (default: 2048).",
    )
    return parser
...
def _eval_prompt_cache_state(prompt_cache):
    """mx.eval only array leaves; TurboQuantKVCache.state mixes dict + arrays."""
    ...
    for c in prompt_cache:
        ...
        if st is None or st == []:
            continue
        arrs = [v for k, v in tree_flatten(st) if isinstance(v, mx.array)]
        if arrs:
            mx.eval(*arrs)
...
def generate_step(
    ...
    *,
    ...
    Args:
        ...
        kv_cache_type (str): ``default`` or ``turboquant`` (hybrid models: TurboQuant on
            attention layers only). Default: ``default``.
    ...
    with mx.stream(generation_stream):
        ...
        while total_prompt_tokens - prompt_processed_tokens > 1:
            ...
            quantize_cache_fn(prompt_cache)
            _eval_prompt_cache_state(prompt_cache)
...
def speculative_generate_step(
    ...
    *,
    ...
    # Create the KV cache for generation
    if prompt_cache is None:
        model_cache = cache.make_prompt_cache(model, kv_cache_type=kv_cache_type)
    ...
    def _prefill(model, cache, y):
        while y.size > prefill_step_size:
            ...
            quantize_cache_fn(cache)
            _eval_prompt_cache_state(cache)
...
def main():
    ...
    if args.memory_mode == "120b-32gb":
        expert_offload = True
        kv_cache_type = "turboquant"
    elif args.memory_mode is not None:
        raise ValueError(
            f'Unknown --memory-mode {args.memory_mode!r}; use "120b-32gb" or omit.'
        )
    ...
    if not using_cache:
        prompt_cache = cache.make_prompt_cache(
            model, max_kv_size=args.max_kv_size, kv_cache_type=kv_cache_type
        )
