...
class ModelArgs(BaseModelArgs):
    ...
    expert_offload_dir: Optional[str] = None
...
class NemotronHMoE(nn.Module):
    def __init__(self, config: ModelArgs, layer_idx: int):
        ...
        self.use_latent = config.moe_latent_size is not None

        # Expert input/output dim: latent_size when latent projections are used
        expert_dim = config.moe_latent_size if self.use_latent else config.hidden_size

        if config.expert_offload:
            quant_cfg = config.quantization if hasattr(config, "quantization") else None
            if quant_cfg is not None:
                # Mixed quantization stores per-module configs keyed by module path.
                fc1_cfg = quant_cfg.get(
                    f"backbone.layers.{self.layer_idx}.mixer.switch_mlp.fc1"
                )
                fc2_cfg = quant_cfg.get(
                    f"backbone.layers.{self.layer_idx}.mixer.switch_mlp.fc2"
                )
                layer_q_cfg = fc1_cfg if isinstance(fc1_cfg, dict) else quant_cfg
                if isinstance(fc2_cfg, dict) and isinstance(layer_q_cfg, dict):
                    if (
                        fc2_cfg.get("bits", layer_q_cfg.get("bits", 4))
                        != layer_q_cfg.get("bits", 4)
                        or fc2_cfg.get("group_size", layer_q_cfg.get("group_size", 64))
                        != layer_q_cfg.get("group_size", 64)
                        or fc2_cfg.get("mode", layer_q_cfg.get("mode", "affine"))
                        != layer_q_cfg.get("mode", "affine")
                    ):
                        raise ValueError(
                            "Nemotron-H quantized expert offload currently requires "
                            "matching fc1/fc2 quantization config per layer."
                        )

                bits = (
                    layer_q_cfg.get("bits", 4) if isinstance(layer_q_cfg, dict) else 4
                )
                group_size = (
                    layer_q_cfg.get("group_size", 64)
                    if isinstance(layer_q_cfg, dict)
                    else 64
                )

                self.switch_mlp = OffloadQuantizedSwitchMLP(
                    expert_dim,
                ...
                self.switch_mlp = OffloadSwitchMLP(
                    expert_dim,
                    config.moe_intermediate_size,
                    config.n_routed_experts,
                    activation=nn.ReLU2(),
    ...
    def __call__(self, x):
        ...
        h = x
        if self.use_latent:
            h = self.fc1_latent_proj(h)

        y = self.switch_mlp(h, inds)
...
class NemotronHBlock(nn.Module):
    def __init__(self, args: ModelArgs, block_type: str, layer_idx: int):
        super().__init__()
...
class Model(nn.Module):
    ...
    def sanitize(self, weights):
        # Drop MTP (multi-token prediction) keys — training-only layers
        ...
        # Stack experts (skipped when routed weights are loaded on demand)
        if not getattr(self.args, "expert_offload", False):
            for l in range(self.args.num_hidden_layers):
                prefix = f"backbone.layers.{l}.mixer"
                for m, n in [("down_proj", "fc2"), ("up_proj", "fc1")]:
                    for suffix in ["weight", "scales", "biases"]:
                        if f"{prefix}.experts.0.{m}.{suffix}" in weights:
                            to_join = [
