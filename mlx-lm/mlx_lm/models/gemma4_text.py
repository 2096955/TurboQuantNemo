...
class DecoderLayer(nn.Module):
    ...
    def __call__(
        ...
        if self.enable_moe:
            ...
            _glu = getattr(self.experts, "switch_glu", None)
            _mgr = (
                getattr(getattr(_glu, "gate_proj", None), "manager", None)
                if _glu
                else None
            )
            if (
                _mgr is not None
                and getattr(_mgr, "dedekimi_observer", None) is not None
            ):
                _mgr.dedekimi_observer.record_activation(
                    self.layer_idx, top_k_indices.reshape(-1)
                )
