...
if not _CLI_HELP_ONLY:
    import mlx.core as mx
    from huggingface_hub import scan_cache_dir

    from ._version import __version__
    from .expert_offload import http_status_for_generation_failure
    ...
    scan_cache_dir = None
    __version__ = "unknown"
    http_status_for_generation_failure = None
    BatchGenerator = None
...
class LRUPromptCache:
    ...
    def _find_checkpoint_positions(self, tokens_len, boundary_positions=None):
        ...
        for interval_start in range(
            self.checkpoint_interval, tokens_len, self.checkpoint_interval
        ):
            # Find message boundaries within this interval
            candidates = [
                p
                for p in boundary_positions
                if last_checkpoint < p < tokens_len
                and p <= interval_start + self.checkpoint_interval // 2
            ]
    ...
    def _extract(self, model, tokens, keep_original=False):
        ...
        if cache_entry.count == 1:
            if keep_original and not is_snapshot:
                ...
                return self.CacheEntry(cache_entry.prompt_cache, 1, cache_entry.nbytes)
    ...
    def fetch_nearest_cache(self, model, tokens):
        ...
        if result.shorter is not None:
            ...
            if is_snapshot:
                ...
                cache_entry = self._extract(
                    result.model, result.shorter, keep_original=False
                )
                logging.debug(
                    f"[LRUPromptCache] CHECKPOINT match: {prefix_len} cached (snapshot), {remaining} to process (requested {requested_len})"
                )
                ...
                cache_entry = self._extract(
                    result.model, result.shorter, keep_original=True
                )
                logging.debug(
                    f"[LRUPromptCache] SHORTER match: {prefix_len} cached, {remaining} to process (requested {requested_len})"
                )
        ...
        if result.longer is not None:
            ...
            if can_trim_prompt_cache(cache_entry.prompt_cache):
                ...
                logging.debug(
                    f"[LRUPromptCache] LONGER match: trimmed {num_to_trim} tokens"
                )
    ...
    def insert_cache(
        self, model, tokens, prompt_cache, boundary_positions=None, old_position=None
    ):
        ...
        if old_position is not None and old_position != tokens:
            ...
            logging.debug(
                f"[LRUPromptCache] Moved cache from {len(old_position)} to {len(tokens)}"
            )
        ...
        if "cache" in current:
            ...
            current["cache"] = self.CacheEntry(
                prompt_cache, current["cache"].count + 1, new_bytes
            )
            self._n_bytes += new_bytes - old_bytes
            self._lru.remove((model, tokens))
            logging.debug(
                f"[LRUPromptCache] Updating cache at position {len(tokens)}, bytes: {old_bytes} -> {new_bytes}"
            )
        else:
            ...
            self._n_bytes += cache_bytes
            logging.debug(
                f"[LRUPromptCache] Adding {cache_bytes} bytes at position {len(tokens)}"
            )

        self._lru.append((model, tokens))

        # Create checkpoint snapshots at interval and message boundaries
        checkpoint_positions = self._find_checkpoint_positions(
            len(tokens), boundary_positions
        )
    ...
    def _create_checkpoint_snapshot(self, model, tokens, prompt_cache, position):
        ...
        current["cache"] = self.CacheEntry(
            snapshot, 1, snapshot_bytes, is_snapshot=True
        )
        self._n_bytes += snapshot_bytes
        self._lru.append((model, checkpoint_tokens))
        logging.debug(
            f"[LRUPromptCache] Created checkpoint snapshot at position {position} ({snapshot_bytes} bytes)"
        )
...
class ModelProvider:
    def __init__(self, cli_args: argparse.Namespace):
        ...
        self.expert_generation_lock = Lock()
    ...
    def load(self, model_path, adapter_path=None, draft_model_path=None):
        ...
        if getattr(self.cli_args, "expert_offload", False) and self.is_distributed:
            raise ValueError(
                "expert_offload is not supported with MLX distributed/tensor-parallel "
                "loading. Run on a single process or disable --expert-offload."
            )
        ...
        if model_path == "default_model":
            ...
            if self.is_distributed:
                ...
                model, tokenizer = load(
                    ...
                    model_config=_model_config_from_cli(self.cli_args),
                )
        else:
            # TODO: Generalize distributed load
            if self.is_distributed:
        ...
        if getattr(self.cli_args, "expert_offload", False):
            self.is_batchable = False
...
class ResponseGenerator:
    def __init__(self, model_provider: ModelProvider, prompt_cache: LRUPromptCache):
        ...
        max_pending = int(
            getattr(model_provider.cli_args, "max_pending_requests", 32) or 32
        )
        self.requests = Queue(maxsize=max(1, max_pending))
        self._queue_put_timeout = getattr(
            model_provider.cli_args, "queue_put_timeout_s", None
        )
    ...
    def _queue_put(
        ...
        *,
        ...
        while not self._stop:
            if ctx is not None and ctx._should_stop:
                return False
            try:
    ...
    def _find_cache_boundaries(self, tokens, tokenizer, min_chunk_size=10):
        """Find positions in tokens where we should create cache boundaries.

        This identifies key positions (like after the system message) where
        storing a separate cache enables better prompt chaining when
        conversations branch in different directions.

        Args:
            tokens: The tokenized prompt
            tokenizer: The tokenizer to use
            min_chunk_size: Minimum tokens before a boundary is considered

        Returns:
            List of positions where cache boundaries should be created
        """
        boundaries = []

        # Find the end-of-message token (typically <|im_end|> or similar)
        end_token = None
        try:
            im_end_tokens = tokenizer.encode("<|im_end|>", add_special_tokens=False)
            if len(im_end_tokens) == 1:
                end_token = im_end_tokens[0]
        except:
            pass

        if end_token is None:
            # Try alternative end tokens
            for end_str in ["</s>", "<|end|>", "[/INST]"]:
                try:
                    end_tokens = tokenizer.encode(end_str, add_special_tokens=False)
                    if len(end_tokens) == 1:
        ...
        # Find positions after each complete message
        # We only store at the FIRST message boundary (after system message)
        # and at the last position (full prompt)
        for i, tok in enumerate(tokens):
            if tok == end_token:
                # Position after this token and any following newline
                pos = i + 1
                # Skip following newline if present
                if pos < len(tokens):
                    try:
                        next_decoded = tokenizer.decode([tokens[pos]])
                        if next_decoded in ["\n", ""]:
                            pos += 1
                    except:
                        pass

                # Only add if it's a significant chunk
                if pos >= min_chunk_size and pos < len(tokens):
                    boundaries.append(pos)
                    # Only store at the first message boundary (usually after system message)
                    # This is the most common branching point
                    break
    ...
    def _generate(self):
        try:
                ...
                if request is not None:
                    ...
                    if (
                        ...
                        if cache is None:
                            cache = make_prompt_cache(
                                ...
                                kv_cache_type=self.model_provider.cli_args.kv_cache_type,
                            )
                            old_position = None
                    ...
                    # serve batched
                        ...
                        batch_generator = BatchGenerator(
                            ...
                            completion_batch_size=self.model_provider.cli_args.decode_concurrency,
                            prefill_batch_size=self.model_provider.cli_args.prompt_concurrency,
                    ...
                    for _ in self._time_budget:
                        ...
                        for r in responses:
                            ...
                            if r.finish_reason is not None:
                                self._queue_put(
                                    result["rqueue"], None, ctx=result["ctx"]
                                )
            ...
            logging.exception("mlx_lm generation worker crashed: %s", e)
    ...
    def _serve_single(self, request):
        ...
        try:
            ...
            if getattr(self.model_provider.cli_args, "expert_offload", False):
                with self.model_provider.expert_generation_lock:
                    for gen in gen_iter:
                        self._queue_put(
                            rqueue,
                            Response(
            ...
            self._queue_put(rqueue, None, ctx=ctx)
            ...
            self._queue_put(rqueue, e, ctx=ctx)
    ...
    def generate(
        ...
        if self._worker_error is not None or not self.worker_alive:
            raise RuntimeError("mlx_lm generation worker is unavailable")

        response_queue = Queue(maxsize=64)
        item = (response_queue, request, generation_args)
        try:
            if self._queue_put_timeout is None or float(self._queue_put_timeout) <= 0:
                self.requests.put_nowait(item)
            else:
                self.requests.put(item, timeout=float(self._queue_put_timeout))
        except QueueFull:
            raise RuntimeError(
                "mlx_lm server request queue is full (increase --max-pending-requests "
        ...
        while True:
            try:
                ctx = response_queue.get(timeout=0.1)
                break
            except QueueEmpty:
                if self._worker_error is not None or not self.worker_alive:
                    raise RuntimeError("mlx_lm generation worker is unavailable")
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def _cors_origin(self) -> Optional[str]:
        raw = getattr(self.response_generator.cli_args, "cors_allow_origin", None)
        if not raw:
            return None
    ...
    def _is_authorized(self) -> bool:
        expected = getattr(self.response_generator.cli_args, "api_key", None)
        if not expected:
            return True
    ...
    def do_POST(self):
        ...
        max_body = int(
            getattr(
                self.response_generator.model_provider.cli_args,
        ...
        if cl_header is None:
            self._write_json_error(411, "Content-Length header required")
            return
        try:
        ...
        if content_length > max_body:
            self._write_json_error(
                413, f"Request body too large (max {max_body} bytes)"
            )
            return
        ...
        try:
            ...
            self._write_json_error(400, f"Invalid JSON in request body: {e}")
            return
        ...
        if not isinstance(self.body, dict):
            self._write_json_error(
                400,
                f"Request body must be a JSON object, got {type(self.body).__name__}",
            )
            return
        ...
        try:
            ...
            logging.warning("Bad request parameters: %s: %s", type(e).__name__, e)
    ...
    def validate_model_parameters(self):
        ...
        if self.stream_options is not None:
            ...
            include_usage = self.stream_options.get("include_usage", False)
    ...
    def handle_completion(self, request: CompletionRequest, stop_words: List[str]):
        ...
        try:
            ...
            err = str(e).lower()
            if "queue is full" in err:
                status = 503
            else:
                status = http_status_for_generation_failure(e)
            logging.error(
                "mlx_lm generation handoff failed [%s]: %s (HTTP %s)",
                type(e).__name__,
                e,
                status,
            )
        ...
        try:
            ...
            if self.stream:
                ...
                self.wfile.write(f"data: {json.dumps(response)}\n\n".encode())
                self.wfile.flush()
                if self.stream_options is not None and self.stream_options.get(
                    "include_usage", False
                ):
    ...
    def do_GET(self):
        ...
        if self.path not in unauthenticated_paths and not self.path.startswith(
            "/health?"
        ):
            if not self._require_authorization():
                return
    ...
    def handle_ready_check(self):
        """Readiness: model and tokenizer loaded (CLI --model preloads by default)."""
        rg = self.response_generator
        mp = rg.model_provider
        ready = (
            mp.model is not None
            and mp.tokenizer is not None
            and rg.worker_alive
            and rg._worker_error is None
        )
        self._set_completion_headers(200 if ready else 503)
        self.end_headers()
        body = {
            "ready": ready,
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional shared bearer token for inference, metrics, and model-list endpoints.",
    )
    parser.add_argument(
        "--cors-allow-origin",
        type=str,
        default=None,
        help="Comma-separated list of allowed browser origins for CORS. Default: disabled.",
    )
    parser.add_argument(
        "--draft-model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Max routed expert tensors resident at once (LRU) when --expert-offload is set.",
    )
    parser.add_argument(
        "--expert-offload-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Max queued generation requests before new requests are rejected or timeout.",
    )
    parser.add_argument(
        "--queue-put-timeout-s",
        type=float,
    ...
    parser.add_argument(
        ...
        help="Reject POST bodies larger than this (default 64 MiB) to limit memory abuse.",
    )
    parser.add_argument(
        "--pipeline",
        action="store_true",
    ...
    if args.host not in {"127.0.0.1", "::1", "localhost"} and not args.api_key:
        logging.warning(
            "Server is bound beyond localhost without --api-key; this is not safe for shared networks."
        )
